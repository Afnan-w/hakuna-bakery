...
const values = [
  { icon: Cake, title: 'Korean Bento Cakes', desc: 'Adorable mini cakes perfect for gifting and intimate celebrations.', color: 'text-hello-pink', bg: 'bg-hello-pink/10' },
  { icon: Star, title: 'Lambeth Piping', desc: 'Vintage royal-inspired buttercream piping that turns cakes into edible art.', color: 'text-hot-pink', bg: 'bg-hot-pink/8' },
  { icon: Heart, title: '100% Eggless Available', desc: 'Every flavor offered in pure eggless preparation, because everyone deserves great cake.', color: 'text-deep-rose', bg: 'bg-deep-rose/8' },
  { icon: Sparkles, title: 'Custom Celebration Cakes', desc: 'From birthdays to weddings, each cake designed to match your vision.', color: 'text-charcoal', bg: 'bg-soft-pink/80' },
  { icon: Leaf, title: 'Artisan Ingredients', desc: 'Belgian chocolate, vanilla bean, pistachio rose, and premium flavors.', color: 'text-hello-pink', bg: 'bg-hello-pink/8' },
  { icon: Palette, title: 'Handcrafted Daily', desc: 'Every cake is baked fresh to order, never sitting on a shelf.', color: 'text-muted-text', bg: 'bg-blush-pink/50' },
]
    ...
    >
      ...
      <section className="relative py-16 sm:py-24 px-4 sm:px-8 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10" ref={ref}>
          ...
          >
            <StrawberrySvg className="w-10 h-10 mx-auto mb-4 opacity-50" />
          </motion.div>
          <motion.h1
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ delay: 0.2, duration: 0.6 }}
            className="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-charcoal"
          >
            Our Story
          </motion.h1>
          <motion.p
            ...
            className="text-muted-text text-sm sm:text-base mt-4 max-w-lg mx-auto leading-relaxed"
          >
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-8 bg-soft-pink/50">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
            ...
            <motion.div
              ...
              transition={{ delay: 0.2, duration: 0.6 }}
            >
              <span className="text-[10px] sm:text-xs font-body font-semibold tracking-[0.3em] uppercase text-hello-pink">
                Since Day One
              </span>
              <h2 className="font-display font-bold text-2xl sm:text-3xl text-charcoal mt-2 mb-5">
                Handcrafted with Love
              </h2>
              <div className="space-y-4 text-muted-text text-sm leading-relaxed">
                <p>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-8 section-glow">
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="text-center mb-12">
            ...
            <h2 className="font-display font-bold text-2xl sm:text-3xl text-charcoal mt-2">
              Our Values
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {values.map((v, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                whileHover={{ y: -4 }}
                className="glass-card rounded-2xl p-6 border border-blush-pink/50 hover:border-hello-pink/30 transition-all duration-300"
              >
                ...
                <h3 className="font-display font-bold text-base text-charcoal mb-2">{v.title}</h3>
                <p className="text-muted-text text-xs leading-relaxed">{v.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-8 bg-soft-pink/40">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-4 sm:gap-5">
            {flavors.map((f, i) => (
              <motion.div
                ...
                whileHover={{ y: -3 }}
                className="flex items-center gap-3 bg-white px-5 py-3.5 rounded-2xl border border-blush-pink/50 hover:border-hello-pink/30 shadow-sm transition-all"
              >
                <div className={`w-6 h-6 rounded-full ${f.color} shadow-sm`} />
                <span className="font-body font-bold text-sm text-charcoal">{f.name}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-8">
        ...
        >
          ...
          <p className="text-muted-text text-sm mb-8 max-w-md mx-auto">
