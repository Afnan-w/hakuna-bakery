    ...
    <>
      <motion.header
        ...
        className={`sticky top-0 z-50 w-full transition-all duration-500 ${
          scrolled
            ? 'bg-cream/95 backdrop-blur-md shadow-[0_2px_20px_rgba(61,35,20,0.06)] border-b border-butter/50'
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 sm:h-18 flex items-center justify-between">
          ...
          <button
            ...
            className="md:hidden p-2 text-chocolate hover:text-cocoa transition-colors cursor-pointer"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed top-16 left-0 right-0 z-40 bg-cream/98 backdrop-blur-lg border-b border-butter/50 shadow-lg overflow-hidden md:hidden"
          >
            <nav className="flex flex-col items-center gap-1 py-4 px-6">
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    to={link.path}
                    className={`text-sm font-body font-bold tracking-[0.2em] uppercase py-3 block transition-colors ${
                      location.pathname === link.path
                        ? 'text-chocolate'
                        : 'text-cocoa/50'
                    }`}
