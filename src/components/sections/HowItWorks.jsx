    ...
    <section ref={ref} className="py-16 sm:py-24 px-4 sm:px-8 relative">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="relative">
          {/* Connector Line */}
          <div className="hidden lg:block absolute top-20 left-[16%] right-[16%] h-[2px] bg-butter">
            <motion.div
              initial={{ scaleX: 0 }}
              animate={isInView ? { scaleX: 1 } : {}}
              transition={{ delay: 0.5, duration: 0.8, ease: 'easeOut' }}
              className="h-full bg-strawberry/30 origin-left"
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            {steps.map((step, i) => (
              ...
              >
                ...
                <h3 className="font-display font-bold text-lg sm:text-xl text-chocolate mb-2">
                  {step.title}
                </h3>
                <p className="text-cocoa/60 text-sm max-w-xs mx-auto leading-relaxed">
                  {step.description}
                </p>
              </motion.div>
            ))}
